...
Public Sub LoadMapColor()
    ...
    If FileExist(DirMapIndex & "MapColor.bin", vbArchive) = False Then
        MsgBox "Falta el archivo 'MapColor.bin' en " & DirMapIndex, vbCritical
        End
    End If
    ...
    Open DirMapIndex & "MapColor.bin" For Binary Access Read As Handle
    Seek Handle, 1
    ...
    For bMapX = 1 To 100
        For bMapY = 1 To 100
            For bCapa = 1 To 3 ' Dibujamos las 3 primeras capas
                tGrh = MapData(bMapX, bMapY).Graphic(bCapa).grh_index
                If tGrh > 0 And tGrh <= lMapColorCount Then
                    SetPixel frmMain.pMiniMap.hDC, bMapX - 1, bMapY - 1, lMapColor(tGrh)
                End If
            Next
            tGrh = MapData(bMapX, bMapY).ObjGrh.grh_index ' Dibujamos los objetos
            If tGrh > 0 And tGrh <= lMapColorCount Then
                tColor = GetPixel(frmMain.pMiniMap.hDC, bMapX - 1, bMapY - 1)
                ...
                SetPixel frmMain.pMiniMap.hDC, bMapX - 1, bMapY - 1, tColor
            End If
            If MapData(bMapX, bMapY).CharIndex > 0 And NPCs = True Then ' Dibujamos los NPC's
                tGrh = CharList(MapData(bMapX, bMapY).CharIndex).Body.Walk(1).grh_index
                If tGrh > 0 And tGrh <= lMapColorCount Then
                    SetPixel frmMain.pMiniMap.hDC, bMapX - 1, bMapY - 1, lMapColor(tGrh)
                End If
            End If
