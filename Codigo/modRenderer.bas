                ...
                If (.GrhIndex <> 0) Then
                    If (.Started = 1) Then
                        If (.SpeedCounter > 0) Then
                            ...
                            If (.SpeedCounter = 0) Then
                                .SpeedCounter = grh_list(.GrhIndex).Speed
                                .FrameCounter = .FrameCounter + 1
                                If (.FrameCounter > grh_list(.GrhIndex).NumFrames) Then _
                                    .FrameCounter = 1
                            End If
                        End If
                    End If
                    'Figure out what frame to draw (always 1 if not animated)
                    iGrhIndex = grh_list(.GrhIndex).Frames(.FrameCounter)
                End If
            End With
            If iGrhIndex <> 0 Then
                rSourceRect.left = grh_list(iGrhIndex).SX
                rSourceRect.top = grh_list(iGrhIndex).SY
                rSourceRect.Right = rSourceRect.left + grh_list(iGrhIndex).pixelWidth
                rSourceRect.Bottom = rSourceRect.top + grh_list(iGrhIndex).pixelHeight
                'El width fue hardcodeado para speed!
                Call BMPSurface.BltFast( _
                        ((32 * ScreenX) - 32) + 0, _
                        ((32 * ScreenY) - 32) + 0, _
                        SurfaceDB.Surface(grh_list(iGrhIndex).FileNum), _
                        rSourceRect, _
                        DDBLTFAST_WAIT)
            End If
            'Layer 2 **********************************
